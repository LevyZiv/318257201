...
const CreateItems= (req,res)=>{
    var Q_create_items = "CREATE TABLE Items (serial_num SMALLINT,item_name VARCHAR(255), category VARCHAR(255), pickup_address VARCHAR(255), size VARCHAR(255), brand VARCHAR(255), manufacture_year SMALLINT, price SMALLINT, seller_email VARCHAR(255),sold BOOLEAN DEFAULT FALSE,buyer_email VARCHAR(255) DEFAULT NULL, order_ID SMALLINT DEFAULT NULL, item_picture, FOREIGN KEY (category) REFERENCES Categories(category), FOREIGN KEY (seller_email) REFERENCES Users(email), FOREIGN KEY (buyer_email) REFERENCES Users(email), FOREIGN KEY (order_ID) REFERENCES Orders(order_ID), PRIMARY KEY (serial_num))";
